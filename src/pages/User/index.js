...
export default function Profile() {
    ...
    useEffect(() => {
        if (!user.loading && !user.error) {
            ...
            if (publicRuntimeConfig.NODE_ENV == 'dev') return
    ...
    return (
        <>
            <Head>
                ...
                <meta
                    name="keywords"
                    content="profile, raka, rakazone, rakazonegaming"
                />
            ...
            <HeaderNotification
                notificationText={
                    'Currently working on this section of this website.'
                }
            />
            <div className={'container-default'}>
                <div className={css.profile_wrapper_grid}>
                    <div
                        className={`${css.left} ${css.common_of_left_right_middle}`}
                    >
                        ...
                        <div className={css.profile_info_wrapper}>
                            <div className={css.profile_info_name}>
                                {userData.name}
                            </div>
                            <div className={css.profile_info_email}>
                                {userData.email}
                            </div>
                            <div className={css.profile_info_created_at}>
                                Joined {userData.created_at}
                            </div>
                        </div>
                        <div>
                            <div className={`${css.achievements_wrapper}`}>
                                Spent 48hrs
                            </div>
                        </div>
                    </div>
                    <div
                        className={`${css.middle} ${css.common_of_left_right_middle}`}
                    >
                        <Input label="Name" />
                        <Input label="Email" />
                        <Input label="DOB" />
                    ...
                    <div
                        className={`${css.right} ${css.common_of_left_right_middle}`}
                    >
                        <div
                            className={css.grey_button}
                            onClick={() => underWork()}
                        >
                            <a>Watch History</a>
                        </div>
