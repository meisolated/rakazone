...
export function VideoPlayerDesktop(props) {
    const src =
        ...
        `internal_api/output/${props.videoId}/HLS/playlist.m3u8`
    const adSrc =
    ...
    const handleTimelineSlider = (e) => {
        ...
        videoController.current.currentTime =
            (percent / 100) * videoController.current.duration
        setDuration({
    ...
    useEffect(() => {
        ...
        videoController.current.addEventListener('timeupdate', () => {
            ...
            setDuration({
                ...
                percentage:
                    (videoController.current.currentTime /
                        videoController.current.duration) *
                    100,
            })
        })
        ...
        document.onkeydown = (e) => {
            if (e.key === ' ') {
                ...
                e.preventDefault()
                videoController.current.currentTime =
                    (0 / 100) * videoController.current.duration
            } else if (e.key == 'm') {
                e.preventDefault()
                handleMute()
                ...
                if (playingAd) return
                videoController.current.currentTime =
                    videoController.current.currentTime - 5
                ...
                if (!videoController.current.muted) {
                    let volume =
                        videoController.current.volume - 0.1 < 0
                            ? 0
                            : videoController.current.volume - 0.1
                    videoController.current.volume = volume
                    let volume_icon = getVolumeIcon(volume * 100)
                    localStorage.setItem('volume', volume)
                    setVolume({
    ...
    useEffect(() => {
        ...
        const rebuild = (video, src) => {
            ...
            hls.once(Hls.Events.LEVEL_LOADED, (event, data) => {
                ...
                setDuration({
                    ...duration,
                    totalDuration: formatDuration(level_duration),
        ...
        video.addEventListener('ended', () => {
            if (video.src === adSrc || hls?.levels[0]?.url[0]?.includes('Ad')) {
                ...
                // timeline Controller
                timelineController.current.addEventListener(
                    'mousedown',
                    (e) => {
    ...
    return (
        <div
            className={desktop_style.main_wrapper}
            onMouseEnter={() => handleControlsState('enter')}
            onMouseLeave={() => handleControlsState('leave')}
        >
            <div
                className={`${
                    !theaterMode
                        ? desktop_style.video_wrapper
                        : desktop_style.theater_mode
                }`}
                ref={videoPlayer}
            >
                {playingAd && (
                    <div className={desktop_style.playing_ad_wrapper}>Ad</div>
                )}
                <div
                    className={`${desktop_style.settings_popup} ${
                        (settingsShowQuality || settingsShowSpeed) &&
                        desktop_style.settings_popup_show
                    }`}
                    style={showSettings ? { display: 'block' } : []}
                >
                    {!settingsShowQuality && !settingsShowSpeed && (
                        <div
                            className={`${desktop_style.settings_item}`}
                            onClick={() => setSettingsShowQuality(true)}
                        >
                            <div
                                className={`${desktop_style.quality_selector} material-icons-round`}
                            >
                                tune
                            </div>
                            Quality{' '}
                ...
                <div
                    className={`${desktop_style.controls_wrapper} ${
                        controlsState && desktop_style.show_controls
                    }`}
                >
                    <div className={desktop_style.controls}>
                        <div className={`${desktop_style.timeline_wrap}`}>
                            <div className={desktop_style.timeline_panel}>
                                <div
                                    ...
                                    className={desktop_style.timeline_slider}
                                >
                                    <div
                                        className={
                                            desktop_style.timeline_slider_track
                                        }
                                    >
                                        <div
                                            className={
                                                desktop_style.timeline_slider_progress
                                            }
                                            style={{
                                                width:
                                                    duration.percentage + '%',
                                                background: playingAd
                                                    ? '#fc0'
                                                    : 'red',
                                            }}
                                        >
                                            <div
                                                className={
                                                    desktop_style.timeline_slider_handle
                                                }
                                            ></div>
                                        </div>
                                    </div>
                        ...
                        <div className={desktop_style.bottom_controls}>
                            <div className={`${desktop_style.controls_left}`}>
                                <div
                                    className={`${desktop_style.pause_play_btn} ${desktop_style.btn}`}
                                >
                                    <div
                                        className={`${desktop_style.play_pause} material-icons-round`}
                                ...
                                <div
                                    className={`${desktop_style.volume_wrap} ${
                                        showVolume
                                            ? desktop_style.volume_show
                                            : []
                                    }`}
                                ...
                                >
                                    <div
                                        className={`${desktop_style.volume_btn} material-icons-round ${desktop_style.btn}`}
                                        onClick={() => handleMute()}
                                    >
                                        {volume.volume_icon}
                                    </div>
                                    <div className={desktop_style.volume_panel}>
                                        ...
                                        >
                                            <div
                                                className={
                                                    desktop_style.volume_slider_track
                                                }
                                            >
                                                <div
                                                    className={
                                                        desktop_style.volume_slider_progress
                                                    }
                                                    style={{
                                                        width:
                                                            volume.volumeLevel +
                                                ...
                                                >
                                                    <div
                                                        className={
                                                            desktop_style.volume_slider_handle
                                                        }
                                                    ></div>
                                                </div>
                                            </div>
                                ...
                                <div className={desktop_style.duration_wrapper}>
                                    <div className={desktop_style.duration}>
                                        <div
                                            className={
                                                desktop_style.duration_current
                                            }
                                        >
                                        ...
                                        <div
                                            className={
                                                desktop_style.duration_separator
                                            }
                                        >
                                            /
                                        </div>
                            ...
                            <div className={desktop_style.controls_right}>
                                <div
                                    className={`${desktop_style.mini_player_container} ${desktop_style.btn}`}
                                >
                                    <div
                                        className={`${desktop_style.settings_btn} material-icons-round`}
                                        style={
                                            showSettings
                                                ? {
                                                      transform:
                                ...
                                >
                                    <div
                                        className={`${desktop_style.mini_player_btn} material-icons-round`}
                                ...
                                >
                                    <div
                                        className={`${desktop_style.theater_btn} material-icons-round`}
                                    >
                                        {theaterMode ? 'crop_7_5' : 'crop_7_5'}
                                    </div>
                                </div>
                                <div
                                    className={`${desktop_style.fullscreen_container} ${desktop_style.btn}`}
                                >
                                    <div
                                        className={`${desktop_style.fullscreen_btn} material-icons-round`}
            ...
            {playingAd && (
                <div className={`${desktop_style.ad_wrapper}`}>
                    ...
                    <div className={desktop_style.ad_text}>
                        boAt Immortal IM-1300 Over-Ear Wireless Gaming Headphone
                        with Mic (Bluetooth 5.1, Driverless 3D Spatial Sound,
                        Black Sabre)
                    </div>
                    <div className={desktop_style.ad_btn_wrapper}>
                        <Primary
                            link="https://www.amazon.in/shop/rakazonegaming"
                            text={'BUY NOW'}
                        />
                    </div>
                </div>
